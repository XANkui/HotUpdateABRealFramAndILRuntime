...
public class BundleHotfix : EditorWindow
{
	...
	static void Init() {
		BundleHotfix window = (BundleHotfix)EditorWindow.GetWindow(typeof(BundleHotfix),false,"热更新包界面",true);
		window.Show();
	}
    ...
    private void OnGUI()
    {
		...
		GUILayout.BeginHorizontal();
		hotCount = EditorGUILayout.TextField("热风补丁版本", hotCount, GUILayout.Width(350), GUILayout.Height(20)) ;
		GUILayout.EndHorizontal();
        if (GUILayout.Button("开始打热更包",GUILayout.Width(100),GUILayout.Height(50)))
        {
            if (string.IsNullOrEmpty(md5Path)==false && md5Path.EndsWith(".bytes"))
            {
